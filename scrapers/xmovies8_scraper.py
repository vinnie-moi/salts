...
class XMovies8_Scraper(scraper.Scraper):
    ...
    def search(self, video_type, title, year):
        ...
        html = self._http_get(search_url, cookies={'location.href': 1}, cache_limit=.25)
        results = []
        match = re.search('>\s*Search Results\s*<', html, re.DOTALL)
        if match:
            pattern = '<h2>\s*<a\s+href="([^"]+)">([^<]+)\s+\((\d+)\)'
            ...
            match = re.search('<link\s+rel="canonical"\s+href="([^"]+)', html)
            if match:
                url = match.group(1)
                result = {'title': title, 'year': year, 'url': url.replace(self.base_url, '')}
                results.append(result)
