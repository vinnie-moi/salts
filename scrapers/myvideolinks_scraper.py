...
class MyVidLinks_Scraper(scraper.Scraper):
    ...
    def search(self, video_type, title, year):
        ...
        pattern ='<h4>\s*<a\s+href="([^"]+)"\s+rel="bookmark"\s+title="([^"]+?)\s*[\[(]?(\d{4})[)\]]?\s*([^"]*)'
        for match in re.finditer(pattern, html, re.DOTALL):
            url, title, year,extra_title  = match.groups('')
            if filter_days:
            ...
            extra_title = extra_title.replace('&#8211;', '-')
            result={'url': url.replace(self.base_url,''), 'title': '%s [%s]' % (title, extra_title), 'year': year}
