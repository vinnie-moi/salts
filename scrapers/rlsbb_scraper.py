...
class RlsBB_Scraper(scraper.Scraper):
    ...
    def get_settings(cls):
        ...
        name = cls.get_name()
        settings.append('         <setting id="%s-filter" type="slider" range="0,180" option="int" label="     %s" default="60" visible="eq(-6,true)"/>' % (name, i18n('filter_results_days')))
        ...
        settings.append('         <setting id="%s-include_comments" type="bool" label="     %s" default="false" visible="eq(-8,true)"/>' % (name, i18n('include_comments')))
        return settings
